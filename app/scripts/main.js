...
function createTimeLineForDay(day,events){
  ...
  var daySegments = 12, startTime = 9, width = 0;
  ...
  for (var i = 0; i < daySegments; i++) {
    ...
    // Create a New Block with the last updated Start time
    var block = {
      startTime : lastUpdatedStartTime,
      description : 'Free Access'
    };

    
    //check if any events are on
    var eventCheck = 0;

    
    for (var x = 0; x < events.length; x++) {
      if(currentTime.isSame(events[x].startTime)){
        block.endTime = events[x].startTime;
        createNewBlock(events[x]);
    ...
    //if event not on make normal block

    //if event then add the last time to the previous block, reset the time for this block, start width again.
